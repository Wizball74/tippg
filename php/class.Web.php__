		...
		function parseSchedule($trid, $md)
		{
			...
			foreach ($matches as $m)
			{
				...
				if ($_sptag > 0)
				{
					...
					if ($res["ResultTypeID"] != 2) $res = $m["MatchResults"][1];
					if ($res["ResultTypeID"] == 2) // 2 == Endergebnis
					{
						$result = sprintf('%d:%d', $res["PointsTeam1"], $res["PointsTeam2"]);
					}
